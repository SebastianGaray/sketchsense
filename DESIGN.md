...
The content width is 70 rem with 1.25/2/3 rem responsive gutters and generous section rhythm. The shell uses one compact header with project identity and an ordered utility group: theme, `EN / ES`, Portfolio return, repository, and Menu. The Menu disclosure owns page navigation and marks the current destination. Stateful controls remain in the header rather than the footer. Theme and navigation menus are viewport-constrained on narrow screens. The hero states what is implemented now before describing the target architecture.
